...
var sha256 = raptoreumcore.crypto.Hash.sha256;
  ...
  if (this.listenAddr) {
    this.on("peeraddr", function peerAddrEvent(peer, message) {
      ...
      for (var i = 0; i < length; i++) {
        ...
        if (
          addr.time.getTime() <= 100000000000 ||
          addr.time.getTime() > future
        ) {
...
Pool.prototype._addAddr = function _addAddr(addr) {
  ...
  // make a unique key
  addr.hash = sha256(Buffer.from(addr.ip.v6 + addr.ip.v4 + addr.port)).toString(
    "hex",
  );
