...
describe('Messages', function() {
  ...
  describe('@constructor', function() {
    it('sets properties correctly', function() {
      var network = raptoreumcore.Networks.defaultNetwork;
      var messages = new Messages({
        network: network,
      ...
      messages.builder.constructors.Block.should.equal(raptoreumcore.Block);
      messages.builder.constructors.Transaction.should.equal(raptoreumcore.Transaction);
      messages.network.should.deep.equal(network);
    });
    it('network should be unique for each set of messages', function() {
      var messages = new Messages({
        network: raptoreumcore.Networks.livenet
      });
      var messages2 = new Messages({
        network: raptoreumcore.Networks.testnet
      });
      messages.network.should.deep.equal(raptoreumcore.Networks.livenet);
      messages2.network.should.deep.equal(raptoreumcore.Networks.testnet);
      var message1 = messages.Version();
      message1.network.should.deep.equal(raptoreumcore.Networks.livenet);
      var message2 = messages2.Version();
      message2.network.should.deep.equal(raptoreumcore.Networks.testnet);
    });
  });
  ...
  describe('Default Network', function() {
    ...
    Object.keys(messages.builder.commandsMap).forEach(function(command) {
      ...
      it(name, function() {
        var message = messages[name]();
        message.network.should.deep.equal(raptoreumcore.Networks.defaultNetwork);
      });
    });
